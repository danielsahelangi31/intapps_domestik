	...
	function __construct( $filename = NULL ) {
		if($filename){
			if(filesize( $filename )){
				...
				throw new Exception('ZERO_LENGTH_FILE', 1002);	
			}
		}
	}
	...
	function load($name){
		if(isset($_FILES[$name]['tmp_name'])){
			...
			if($filename){
				...
				throw new Exception('NULL_FILENAME', 1001);	
			}
		}
	}
...
//			echo $s['r:id'].'<br /';
	...
	//			$bE = ($aP['GPF'] && 0x0001) ? TRUE : FALSE;
			...
			throw new Exception("FAILED_TO_UNZIP", 201);	
		}
	}
	function getPackage() {
		return $this->package;
	...
	function _parse() {
		...
		if($relations = simplexml_load_string( $this->getEntryData("_rels/.rels") )){
			foreach ($relations->Relationship as $rel) {
				if ($rel["Type"] == SimpleXLSX::SCHEMA_OFFICEDOCUMENT) {
					// Workbook
					if($this->workbook = simplexml_load_string( $this->getEntryData( $rel['Target'] ) )){					
						// Found office document! Read relations for workbook...
						if($workbookRelations = simplexml_load_string($this->getEntryData( dirname($rel['Target']) . '/_rels/workbook.xml.rels') )){
							...
							foreach ($workbookRelations->Relationship as $workbookRelation) {
								...
								if ($workbookRelation['Type'] == SimpleXLSX::SCHEMA_WORKSHEETRELATION) { // Sheets
									...
									$this->sheets[ str_replace( 'rId', '', (string) $workbookRelation['Id']) ] = simplexml_load_string( $this->getEntryData( $path ) );
			
								} else if ($workbookRelation['Type'] == SimpleXLSX::SCHEMA_SHAREDSTRINGS) {
									
									$sharedStrings = simplexml_load_string( $this->getEntryData( $path ) );            
									
									if (isset($sharedStrings->si)) {
							...
							throw new Exception("WORKBOOK_NOT_FOUND", 103);							
						}
					}else{
						throw new Exception("NOT_EXCEL_2007_DOCUMENT", 102);	
					}
			...
			throw new Exception("UNKNOWN_FILE", 101);
		}
