...
<body>
	...
	<div id="wrap">
		...
		<div class="container">
             ...
             <h1 style="margin-bottom:25px;margin-left:-25px">Laporan YOR</h1>
            ...
            <div class="d-flex justify-content-left">
                    <div class="col-md-6">
                        ...
                        <div class="form-group" style="margin-left:0px;width:48.5%;"> 
                            ...
                            <input type="date" class="form-control" id="PERIODE_AWAL" name="PERIODE_AWAL" value="PERIODE_AWAL"/>                            
                        </div>
                        
                        <div class="form-group" style="margin-left:0px;width:48.5%;"> 
                            <label class="col-form-label">Periode Akhir<b class="text-danger">*</b></label>
                            <input type="date" class="form-control" id="PERIODE_AKHIR" name="PERIODE_AKHR"/>                            
                        </div>   
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
            ...
            $('#simpan').click(function(e){
                ...
                console.log('bln_akhir',$blnAkhir);
                console.log('err', is_error);
                if(is_error){      
                ...
                }else if ($blnAwal !== $blnAkhir){
                    $.notify("Bulan berjalan harus sama","error");               
				}else{
				console.log(param);
                var PERIODE_AWAL = $('#PERIODE_AWAL').val();
                var PERIODE = new Date(PERIODE_AWAL);
                var start = moment(PERIODE).format('YYYY-MM-DD'); ;
                console.log('awal',start);
         
                var PERIODE_AKHIR = $('#PERIODE_AKHIR').val();
                var AKHIR = new Date(PERIODE_AKHIR);
                var end = moment(AKHIR).format('YYYY-MM-DD'); ;
                var terminal = $('#TERMINAL').val();
                var urls = bs.baseURL + 'tps_online/laporan_yor/export_yor_xls/'+start+'/'+end
                console.log('end',end)
           
                e.preventDefault(); 
                if (terminal == 'DOM'){
                    window.location.href = bs.baseURL + 'tps_online/laporan_yor/export_yor_xls/'+start+'/'+end+'/'+terminal;
                }else if (terminal == 'INT') {
                    window.location.href = bs.baseURL + 'tps_online/laporan_yor/export_yor_intr/'+start+'/'+end+'/'+terminal;
                }
