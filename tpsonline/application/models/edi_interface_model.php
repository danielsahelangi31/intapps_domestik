...
class Edi_Interface_Model extends ModelBase{
	...
	/** Query dataset untuk datagrid (listview)
	  * /!\ Jangan ubah struktur data keluaran
	  */
	public function select(){
		...
		$out = new StdClass();			
		// Sesuaikan query, sebisa mungkin gunakan CI active records 
	...
	/** Untuk mendapatkan single row data, misalnya untuk query ketika view / edit
	  */
	...
	public function save_invoice($id, $member_id, $invoice, $flag_lunas){
		...
		$nota = array(
			'member_id' => $member_id,
			'nomor_faktur_pajak' => $header->noFakturPajak,
	...
	public function assign_truck($id, $auth){
		...
		$det = $this->db->select('ogdrl.id, ogdr.rencana_ambil, cl.*, cd.consignee, tp.nama_terminal_petikemas')
						->join('terminal_petikemas tp', 'tp.id = ogdr.terminal_petikemas_id')
