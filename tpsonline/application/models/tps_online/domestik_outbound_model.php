...
class Domestik_Outbound_Model extends ModelBase {
    ...
    public function select($users_id) {
        ...
        $out = new StdClass();
//        $out->datasource = $this->db->where('VISIT_DIRECTION', '2')->order_by('ETA', 'DESC')->get('CARTOS_SHIP_VISIT sv')->result();
        $out->datasource = $this->db->distinct()->select('A.*')
                ->from('CARTOS_SHIP_VISIT A')
                ->join('CARTOS_CARGO B', 'A.VISIT_ID = B.VISIT_ID', 'left')
                ->where('A.VISIT_DIRECTION', '2')
