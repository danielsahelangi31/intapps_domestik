...
class Model_rkap_arus_barang extends ModelBase {
	...
	public function select($users_id) {
        ...
        $out->databarang = $this->db->distinct()->select('TERMINAL,JENIS,TAHUN,KOMODITI,SATUAN,JANUARI,FEBRUARI,MARET,APRIL,MEI,JUNI,JULI,AGUSTUS,SEPTEMBER,OKTOBER,NOVEMBER,DESEMBER,id_barang') 
        ...
        $this->siapkanDB(true);
            ...
            $out->actualRows = $this->db->select('count("id_barang") AS "numRows"', FALSE)
            ->get('DASHBOARD_RKAP_ARUS_BARANG')
            ->row()->numRows;
    ...
    public function get($id) {
        return $this->db
                        ->where('id_barang', $id)
                        ->get('DASHBOARD_RKAP_ARUS_BARANG')->row();
    }
