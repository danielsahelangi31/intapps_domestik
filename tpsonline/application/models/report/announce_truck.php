...
class Announce_truck extends CI_Model {
    ...
    function _get_datatables_query($maker, $bulan, $tahun) {
        ...
        $column_order = array(null,'LICENCE_PLATE', 'RESPON_CODE', 'RESPON_MSG', 'SENDER', 'RECORD_TIME');
        $column_search = array('LOWER(LICENCE_PLATE)','LOWER(SENDER)','LOWER(RESPON_CODE)');
        $order = array('LICENCE_PLATE' => 'asc');

        $this->db2->select('LICENCE_PLATE, SENDER, RESPON_CODE, RESPON_MSG, RECORD_TIME');
        $this->db2->from($table);
        if($maker == 'OTHER'){
    ...
    function chart_maker($maker, $bulan, $tahun){
        ...
        $query = "SELECT DISTINCT (SELECT COUNT(*) FROM ANNOUNCETRUCK_REQ WHERE RESPON_CODE = '200' AND SENDER LIKE '%".$maker."%' ".$where_bulan." ".$where_tahun.") AS Sukses, 
                    (SELECT COUNT(*) FROM ANNOUNCETRUCK_REQ WHERE (RESPON_CODE != '200' OR RESPON_CODE = '' OR RESPON_CODE IS NULL) AND SENDER LIKE '%".$maker."%' ".$where_bulan." ".$where_tahun.") AS Gagal
                    FROM ANNOUNCETRUCK_REQ";
        if($maker == 'OTHER')
            $query = "SELECT DISTINCT (SELECT COUNT(*) FROM ANNOUNCETRUCK_REQ WHERE RESPON_CODE = '200' AND (SENDER NOT LIKE '%NSDS%' AND SENDER NOT LIKE '%EVLS%' AND SENDER NOT LIKE '%MMKI%' AND SENDER NOT LIKE '%ADLES%') ".$where_bulan." ".$where_tahun.") AS Sukses, 
                    (SELECT COUNT(*) FROM ANNOUNCETRUCK_REQ WHERE (RESPON_CODE != '200' OR RESPON_CODE = '' OR RESPON_CODE IS NULL) AND (SENDER NOT LIKE '%NSDS%' AND SENDER NOT LIKE '%EVLS%' AND SENDER NOT LIKE '%MMKI%' AND SENDER NOT LIKE '%ADLES%') ".$where_bulan." ".$where_tahun.") AS Gagal
                    FROM ANNOUNCETRUCK_REQ";
