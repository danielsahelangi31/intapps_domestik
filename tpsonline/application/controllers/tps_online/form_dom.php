<?php
/** Input Manual Monitoring BM 
  *	Modul untuk menambahkan activity monitoring bm domestik tiap kapal dan laporan activity bm domestik
  *
  */

class form_dom extends CI_Controller{
	private $local_db;
	...
	public function index(){
		redirect('tps_online/form_dom/listview');
	}
	...
	public function listview(){	
		...
		$this->load->view('backend/pages/tps_online/form_dom/listview',$data);
	}
    ...
    public function view($id = NULL) {
        ...
        if (!$grid_state) {
            $grid_state = 'tps_online/form_dom/listview';
        }
        ...
        if ($row = $mod->get($id)) {
			...
			$data['kunjung'] = $row;	
            $this->load->view('backend/pages/tps_online/form_dom/view', $data);
    ...
    public function finalize($id = NULL, $voyage = NULL) {
        ...
        if (!$grid_state) {
            $grid_state = 'tps_online/form_dom/finalize';
        }
		...
		$data['databm'] = $databm;	
		 ...
		 $view = array(			
            ...
            $this->load->view('backend/pages/tps_online/form_dom/finalize', $view);
  	  ...
  	  public function new($id = NULL, $voyage = NULL) {
			...
			ORDER BY "ID_MONITORING_HEADER" ASC
			';
			...
			if ($COMPLETEH == '1970-01-01T07:00:00'){
			 ...
			 $dataField = array('ID_VESSEL'=>'', 'VESSEL_NAME'=>'', 'VOY_IN'=>'', 'PBM'=>'', 'ETA'=>'');
            ...
            $this->load->view('backend/pages/tps_online/form_dom/new', $data);
			}
    }
	...
	public function neww() {
		...
		$voyage = $_REQUEST['voyage'];
		 ...
		 // // Content Data
		$datasource_modal = $this->db->select('ID_VESSEL, VESSEL_NAME, VOY_IN, PBM, ETA')->order_by("ETA", "DESC")->get('STAGING_VES_VOYAGE')->result();

		$datasource = $db->select('ID_VESSEL, VESSEL_NAME, VOY_IN, PBM, ETA')->where('VESSEL_NAME',$vesse_id)->where('VOY_IN',$voyage)->get('STAGING_VES_VOYAGE')->result();
	
		$dataField = array('ID_VESSEL'=>'', 'VESSEL_NAME'=>'', 'VOYAGE'=>'', 'PBM'=>'', 'ETA'=>'');
		...
		$this->db->where("VESSEL_NAME",$vesse_id);

		$data= array('view'=>$view, 'VESSEL_NAME'=>$vesse_id);	
	...
	public function get_vesel($token = NULL){
		if($this->auth->token == $token){
			...
			$data = $mod->getVESEL($where['VESSEL_NAME']);
    ...
    public function export_form_xls($id,$voyage)
	{
			...
			foreach($model['data'] as $data){ // Lakukan looping pada variabel siswa
				...
				$excel->setActiveSheetIndex(0)->setCellValue('B5', $kade);
				...
				$excel->setActiveSheetIndex(0)->setCellValue('B9', $data['PBM']);	
			...
			header('Content-Disposition: attachment; filename="BM_MONITORING_DOM_'.$id.'_'.$voyage.'.xls"'); // Set nama file excel nya
