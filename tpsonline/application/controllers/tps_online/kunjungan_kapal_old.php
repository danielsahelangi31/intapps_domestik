	...
	public function view($id = NULL){
		...
		if($row = $mod->get($id)){
			if(is_post_request()){
				...
				$val->set_rules('BC_NUMBER', 'Nomor BC 1.1', 'required');
				$val->set_rules('BC_DATE_NUMBER', 'Tanggal BC 1.1', 'required');
				...
				$row->BC_NUMBER = post('BC_NUMBER');
				$row->BC_DATE_NUMBER = post('BC_DATE_NUMBER');
