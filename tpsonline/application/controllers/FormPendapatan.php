<?php
/** Form RKAP Pendapatan 
  *	Modul untuk menyimpan dan edit data pendapatan perpusat layanan berdasarkan tahun
  *
  */

class FormPendapatan extends CI_Controller{
	private $local_db;
		...
		if($this->auth->token == $token){
			...
			if(isset($_REQUEST)){
				...
				if($val->run()){
				   ...
				   $dataIdpendapatan = 'SELECT max("id_pendapatan") FROM "DASHBOARD_RKAP_PENDAPATAN"';
				   $data = $con->query($dataIdpendapatan)-> row();
				   $out->datazero=$data;
				   $array = json_encode($data);
				   $x = json_decode($array);
				   $y = $x->max;
				   $barang = $y+1;				
                   $id_pendapatan =  "'$barang'"; 
                   
				   $databarang = 'SELECT "PELAYANAN", "GOLONGAN", "TAHUN", "KOMODITI"
				   ...
				   for ($x = 0; $x < $cont; $x++) {
					...
					$tm1 = $databarang[$x]['PELAYANAN'];
					$j1 = $databarang[$x]['GOLONGAN'];
					...
					if ($tm1 == $pelayanan && $j1 == $golongan && $km1 == $komoditi && $th1 == $tahun){
						$dat = 'sama';
					...
					$query_header = 'INSERT INTO "DASHBOARD_RKAP_PENDAPATAN"(
									"PELAYANAN",
									"TAHUN",
									"GOLONGAN",
									"KOMODITI",								
		...
		if($this->auth->token == $token){
			...
			if(isset($_REQUEST)){
				...
				if($val->run()){
					...
					$id_pendapatan = post ('id_pendapatan');
					...
					$query_header = 'UPDATE "DASHBOARD_RKAP_PENDAPATAN"
									SET 									
									"PELAYANAN" = '.$pelayanan.',
									"TAHUN" = '.$tahun.',
									"GOLONGAN" = '.$golongan.',
