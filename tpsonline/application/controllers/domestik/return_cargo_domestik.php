...
class Return_Cargo_Domestik extends CI_Controller {
    ...
    public function index(){
        $this->load->view('domestik/backend/pages/eticket/return_cargo/request_return_cargo_domestik');
    }

    public function getTruckCodeList()
    {
        $mod = model('domestik/rc_model_domestik');
    ...
    public function getTableItems()
    {
        ...
        $response = $mod->getTableItems($searchTerm);
        $data = [];
        foreach ($response as $field) {
            $checkbox_action = '<td>
            <input type="checkbox" id='."'".$field->VIN_NEW."'".' class="checkSingle" value='."'".$field->VIN_NEW."'".'/>
            </td>';
            $row = array();
            $row[] = $checkbox_action;
            $row[] = $field->VIN_NEW;
            $row[] = $field->MODEL_NEW;
            // $row[] = $field->MODEL_CODE;
            // $row[] = $field->SHIPPING_CODE_NEW;
            $row[] = $field->SHIPPING_NAME_NEW;
            $row[] = $field->VESSEL_NAME_NEW == " " ? 'No Data' : $field->VESSEL_NAME_NEW;
            $row[] = $field->STATUS_NEW;
            // $row[] = $field->DOC_TRANSFERID;
            // $row[] = $field->VESSEL_CODE_NEW == " " ? 'No Data' : $field->VESSEL_CODE_NEW;
    ...
    public function submitItems()
    {
        ...
        $new_ktp_name = 'KTP_'.pathinfo($_FILES["browse_ktp_sim"]['name'], PATHINFO_FILENAME).'_'.time();
        $new_srj_name = 'SRJ_'.pathinfo($_FILES["browse_surat_jalan"]['name'], PATHINFO_FILENAME).'_'.time();
        $data = post();
        $uploadError = array();
        ...
        $configUpload['upload_path']    = './assets/documents/ktp/';
        ...
        $configUpload['file_name']   = $new_ktp_name;    
        $this->load->library('upload', $configUpload, 'ktpupload');                 
      
        $this->ktpupload->initialize($configUpload);
        if(!$this->ktpupload->do_upload('browse_ktp_sim')){
            $uploadedKTP    = "".$this->ktpupload->display_errors()." (KTP)";
            $uploadError[] = array('message' => $uploadedKTP, 'isError' => true);
        }else{
            $uploadedKTP    = $this->ktpupload->data();    
        }
        
        if(!$uploadError){
            $configUpload2['upload_path']    = './assets/documents/suratjalan/';
            $configUpload2['allowed_types']  = 'jpg|jpeg|png|pdf';  
            $configUpload2['max_size']       = '0';                          
            $configUpload2['max_width']      = '0';                          
            $configUpload2['max_height']     = '0';                         
            $configUpload2['encrypt_name']   = false;                      
            $configUpload2['file_name']   = $new_srj_name;    
            $this->load->library('upload', $configUpload2, 'srjupload');                 
        
            $this->srjupload->initialize($configUpload2);
            if(!$this->srjupload->do_upload('browse_surat_jalan')){
                $uploadedSRJ    = "".$this->srjupload->display_errors()." (SURAT JALAN)";
                $uploadError[] = array('message' => $uploadedSRJ, 'isError' => true);
            }else{
                $uploadedSRJ    = $this->srjupload->data();    
            }
        }
        

        if(!$uploadError){  
            $data['ktp_path'] = explode('tpsonline/', $uploadedKTP["full_path"])[1];
            $data['srj_path'] = explode('tpsonline/', $uploadedSRJ["full_path"])[1];


            if($data['return_type'] == "Truck"){
                $response = $mod->insertRCTruckData($data);
                echo json_encode($response);
        }else{
                $response = $mod->insertRCSelfdriveData($data);
                echo json_encode($response);
        }
        } else {
